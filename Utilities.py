...
def buildWebServerFromDockFile(filePath):
	...
	try:
		for line in lowLevelClient.build(path=filePath,decode=True,tag="web_server"):
		    print line
...
def startMongodbContainer():
	mongodbContainer = client.containers.run('mongo',name="mongodb",ports={'27017/tcp':27017},
		volumes={'/data/db/': {'bind': '/data/db', 'mode': 'rw'}},detach=True,remove=True)

def startWebServer():
	webServerContainer = client.containers.run("web_server",name="web",ports={'3000/tcp':3000},
	links={'mongodb':'mongodb'},remove=True,detach = True)

	print webServerContainer.logs()

def runOutputLogs():
	for line in mongodbContainer.attach(stdout = True,stderr =True,stream =True):
 		print line
